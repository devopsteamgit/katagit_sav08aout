...
docker exec  dev1_git_local bash -c "su - git -c \"cd PROJET;cp ../repofiles/script_1.sh.;git add --all ;git commit -a -m 'ajout script_1.sh'\"";
docker exec  dev1_git_local bash -c "su - git -c \"cd PROJET;cp ../repofiles/script_2.sh.;git add --all ;git commit -a -m 'ajout script_2.sh'\"";
