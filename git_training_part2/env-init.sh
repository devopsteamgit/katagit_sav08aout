...
docker exec dev1_git_local bash -c "su - git -c \"git init;git config --global user.email \"git@example.com\";git config --global user.name \"git Name\";pwd;git config --list;git add . ;git commit -a -m \"creation branch master \"; mkdir PROJET";
